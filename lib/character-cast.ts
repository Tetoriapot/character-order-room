...
export const PERSON_CUSTOM_KEYS = ['character', 'appearance', 'outfit', 'action'];
...
export function captureMember(snapshot: CharacterSnapshot, metadata: Pick<CastMember, 'id' | 'name' | 'position'>): CastMember {
  return {
...
export function memberDraft(draft: CharacterDraft, member: CastMember): CharacterDraft {
  return { ...draft, ...member.fields, cast: undefined, generatedGap: member.generatedGap,
    custom: { ...draft.custom, ...member.custom } };
}
...
export function setCastEnabled(source: CharacterSnapshot, enabled: boolean): CharacterSnapshot {
  ...
  const first = captureMember(snapshot, { id: 'person-1', name: '', position: 'left' });
  const second = captureMember({ draft: blankPerson(snapshot.draft), locks: {} }, { id: 'person-2', name: '', position: 'right' });
...
export function addCastMember(source: CharacterSnapshot): CharacterSnapshot {
  ...
  let serial = 1;
  while (cast.members.some((item) => item.id === `person-${serial}`)) serial++;
  const member = captureMember({ draft: blankPerson(snapshot.draft), locks: {} }, { id: `person-${serial}`, name: '', position: '' });
...
export function hydrateCast(value: unknown, hydratePerson: (value: unknown) => CharacterDraft): CharacterCast | undefined {
  ...
  for (const item of source.members.slice(0, MAX_CAST_MEMBERS)) {
    ...
    const id = typeof row.id === 'string' && /^[a-zA-Z0-9_-]{1,64}$/.test(row.id) ? row.id : `person-${members.length + 1}`;

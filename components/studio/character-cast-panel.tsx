...
export function CharacterCastPanel({ snapshot, language, onChange }: {
  ...
  if (backgroundOnly) return <section className="mb-4 rounded-xl border border-border bg-card p-3 text-sm text-muted-foreground">{tr(`背景のみを出力中。人物設定${cast?.members.length ?? 1}人分は保持しています。用途を戻すと再編集できます。`, `Outputting background only. Settings for ${cast?.members.length ?? 1} people are retained; change purpose to edit them again.`)}</section>;
    ...
    {backgroundOnly ? <p className="mt-3 text-sm text-muted-foreground">{tr('背景用途では人物を出力しません。人物設定は保持されます。', 'Background mode omits people but keeps their settings.')}</p>
      ...
      : <>
        ...
        <details className="mt-3 rounded-xl border border-border p-3">
          ...
          <div className="mt-4 grid grid-cols-3 gap-2 rounded-xl border border-border p-3 text-center text-sm" role="img" aria-label={tr('画面から見た配置：左・中央・右', 'Image-relative placement: left, center, right')}>{(['left', 'center', 'right'] as const).map((position) => <div key={position}><p className="font-bold">{castChoiceLabel(castPositions, position, language)}</p><p className="mt-2 break-words text-muted-foreground">{cast.members.filter((member) => member.position === position).map((member) => member.name || tr(`人物${cast.members.indexOf(member) + 1}`, `Person ${cast.members.indexOf(member) + 1}`)).join(' / ') || '—'}</p></div>)}</div>

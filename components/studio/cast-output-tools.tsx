...
export function CastOutputTools({ snapshot, language, onCopy, onJump }: {
  ...
  const untranslated = useMemo(() => {
    ...
    return entries;
  }, [draft, parts, language, tr]);
  return <div className="mt-4 space-y-3">
    {checks.length > 0 && <section className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4"><h3 className="text-sm font-bold">{tr('コピー前の確認', 'Before you copy')}</h3><p className="mt-1 text-sm text-muted-foreground">{tr('自動修正はしません。意図どおりなら、そのままコピーできます。', 'Nothing is automatically changed. Copy as-is if intentional.')}</p><ul className="mt-3 space-y-3">{checks.map((check) => <li key={check.id} className="text-sm">{check[language]}{check.personId && <Button variant="link" className="min-h-11" onClick={() => onJump(check.personId, 'character')}>{tr('人物を編集', 'Edit person')}</Button>}</li>)}</ul></section>}
    {parts && <details className="rounded-lg border border-border bg-card p-4"><summary className="min-h-11 cursor-pointer text-sm font-bold">{tr('共通部分・人物別にコピー', 'Copy shared and person blocks')}</summary>

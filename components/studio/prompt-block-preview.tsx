  ...
  return <div className="space-y-3">
    ...
    {promptBlockNames.map((name) => {
      ...
      return <div key={name} className="border-t border-border pt-2">
        ...
        {included && blocks[name] ? <p className="whitespace-pre-wrap [overflow-wrap:anywhere] text-sm leading-7" lang={name === 'CONTENT' ? 'ja' : 'en'}>{blocks[name]}</p> : <p className="text-xs text-muted-foreground">{!blocks[name] ? tr('未設定（コピーされません）', 'Not set (not copied)') : tr('非表示・コピー対象外', 'Hidden and excluded from copy')}</p>}
      </div>;
    })}
    <div className="flex flex-wrap gap-2">
      <Button variant="outline" className="min-h-11 rounded-lg" disabled={!formatted} onClick={() => onCopy(formatPromptBlocks(blocks, selection.excludedBlocks, 'line'), tr('1行形式をコピー', 'Copied one-line format'))}>{tr('1行コピー', 'Copy one line')}</Button>
      <Button variant="outline" className="min-h-11 rounded-lg" disabled={!formatted} onClick={() => onCopy(formatPromptBlocks(blocks, selection.excludedBlocks, 'json'), tr('プロンプトJSONをコピー', 'Copied prompt JSON'))}>{tr('プロンプトJSON', 'Prompt JSON')}</Button>
    </div>

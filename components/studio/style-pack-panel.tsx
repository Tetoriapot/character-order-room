...
export function StylePackPanel({ selection, language, favorites, locked, onChange, onToggleFavorite, onSaveTemplate, onPreview }: Props) {
  ...
  return (
    <section className="min-w-0 space-y-4" aria-label={tr('画風ライブラリ', 'Style library')}>
      <div>
        <h3 className="font-bold">{tr('画風ライブラリ', 'Style library')} <span className="ml-2 text-sm font-normal text-muted-foreground">80 STYLE</span></h3>
        <p className="mt-1 text-sm leading-relaxed text-muted-foreground">{tr('画風を1つ選び、内容と分けて指示できます。従来の絵柄設定も下に残しています。', 'Choose one style and keep it separate from the content. Classic style controls remain available below.')}</p>
      </div>
      <div className="flex flex-wrap gap-2">
        <label className="relative min-w-0 flex-1 basis-48">
          <Search className="pointer-events-none absolute left-3 top-3.5 size-4 text-muted-foreground" />
          <Input className="h-11 rounded-xl pl-9" value={query} onChange={(event) => setQuery(event.target.value)} aria-label={tr('画風を検索', 'Search styles')} placeholder={tr('名前・用途・タグで検索', 'Search name, use case, or tag')} />
        </label>
        <Button variant={favoritesOnly ? 'secondary' : 'outline'} aria-pressed={favoritesOnly} className="min-h-11 rounded-xl" onClick={() => setFavoritesOnly(!favoritesOnly)}><Star className="size-4" />{tr('お気に入り', 'Favorites')}</Button>
      </div>
      <Tabs value={category} onValueChange={(next) => setCategory(next as StyleCategory | 'all')}>
        <TabsList aria-label={tr('画風カテゴリ', 'Style categories')} className="w-full flex-wrap justify-start gap-1 bg-muted/60 group-data-horizontal/tabs:h-auto">
        ...
        <TabsContent value={category}>
          <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
            ...
            <Button variant="ghost" className="min-h-11 rounded-xl" disabled={locked} onClick={() => { const next = pickRandomStyle(category, value.presetId); if (next) select(next.id); }}><Dices className="size-4" />{tr('このカテゴリからランダム', 'Random in category')}</Button>
          </div>
          ...
          <div className="max-h-80 overflow-y-auto overscroll-contain rounded-xl border border-border p-2" tabIndex={0} role="region" aria-label={tr('画風の候補一覧', 'Style results')}>
            <div className="grid gap-2 sm:grid-cols-2">
              {presets.map((preset, index) => {
                const active = value.presetId === preset.id;
                const favorite = favorites.includes(preset.id);
                return <div key={preset.id} className={`relative min-w-0 rounded-xl border ${active ? 'border-primary bg-primary/5' : 'border-border bg-card'}`}>
                  <button type="button" className="h-full min-h-24 w-full rounded-xl p-3 pr-12 text-left outline-offset-2 focus-visible:outline-2 focus-visible:outline-ring" aria-pressed={active} aria-label={tr(preset.nameJa, preset.nameEn)} onClick={() => select(preset.id)}>
                  ...
                  <Button variant="ghost" size="icon" className="absolute right-0 top-0 size-11 rounded-xl" aria-pressed={favorite} aria-label={tr(`${preset.nameJa}をお気に入り${favorite ? 'から外す' : 'に追加'}`, `${favorite ? 'Unfavorite' : 'Favorite'} ${preset.nameEn}`)} onClick={() => onToggleFavorite(preset.id)}><Star className={`size-4 ${favorite ? 'fill-primary text-primary' : 'text-muted-foreground'}`} /></Button>
                </div>;
              })}
            </div>
      ...
      {style && <div className="space-y-3 rounded-xl border border-primary/25 bg-primary/5 p-4">
        <p className="font-semibold">{tr('選択中：', 'Selected: ')}{tr(style.nameJa, style.nameEn)}</p>
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" className="min-h-11 rounded-xl" onClick={onPreview}>{tr('ブロックを確認', 'Preview blocks')}</Button>
          <Button variant="outline" className="min-h-11 rounded-xl" onClick={onSaveTemplate}>{tr('テンプレ保存', 'Save template')}</Button>
          <Button variant="ghost" className="min-h-11 rounded-xl" onClick={() => select('')}>{tr('解除して従来の絵柄へ', 'Use classic style')}</Button>
        </div>
      </div>}
      {style && <div className="space-y-3">
        <div>
        ...
        <div className="rounded-xl bg-muted/60 p-3">
          ...
          <Button variant="outline" className="mt-2 min-h-11 rounded-xl" onClick={() => onChange({ ...value, antiAiIds: recommendations.map((item) => item.id) })}>{tr('補助をおすすめ3件に置換', 'Replace helpers with these 3')}</Button>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-sm text-muted-foreground">{tr('補助を置換：', 'Replace helpers: ')}</span>
          {(['weak', 'medium', 'strong'] as const).map((level, index) => <Button key={level} variant="outline" className="min-h-11 rounded-xl" onClick={() => onChange({ ...value, antiAiIds: [...antiAiStrengths[level]] })}>{tr(['弱（2件）', '中（3件）', '強（5件）'][index], `${level} (${antiAiStrengths[level].length})`)}</Button>)}
          <Button variant="ghost" className="min-h-11 rounded-xl" disabled={!value.antiAiIds.length} onClick={() => onChange({ ...value, antiAiIds: [] })}>{tr('補助を解除', 'Clear helpers')}</Button>
        </div>
        {!!value.antiAiIds.length && <div className="flex flex-wrap gap-2" aria-label={tr('選択中の補助', 'Selected helpers')}>
          {value.antiAiIds.map((id) => { const block = findAntiAiBlock(id); return block ? <Button key={id} variant="secondary" className="h-auto min-h-11 max-w-full whitespace-normal rounded-xl py-2 text-left" aria-label={tr(`${block.nameJa}を解除`, `Remove ${block.nameEn}`)} onClick={() => onChange({ ...value, antiAiIds: value.antiAiIds.filter((item) => item !== id) })}>{tr(block.nameJa, block.nameEn)}<X className="size-3.5" /></Button> : null; })}
        </div>}
        {warnings.map((warning) => <p key={warning.en} className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-sm" role="status">{tr(warning.ja, warning.en)}</p>)}
        <details className="rounded-xl border border-border p-3">
          ...
          <div className="max-h-96 space-y-2 overflow-y-auto overscroll-contain pr-1" tabIndex={0} role="region" aria-label={tr('画風の補助一覧', 'Style helper list')}>
            {antiAiBlocks.map((block) => {
              ...
              return <label key={block.id} className={`flex min-h-11 cursor-pointer items-start gap-3 rounded-xl border p-3 ${selected ? 'border-primary/40 bg-primary/5' : 'border-border'} ${disabled ? 'opacity-60' : ''}`}>

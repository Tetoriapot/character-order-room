...
STEP 7「カメラと構図」で、カメラ角度30種・構図35種からそれぞれ1つを選べます。背面や左右からの視点、上方・下方からの視点、余白、三分割配置、前景・中景・遠景などを追加しています。検索・カテゴリ・お気に入り・最近から候補を探せます。

左右の指定は人物を基準にしたカメラ位置です。背景用途では、背景にも使える配置・余白・奥行きを出力します。人物専用の構図は保持されますが背景出力には含まれず、背景のランダム生成でも選びません。既存の選択内容、ロック、保存・共有形式は維持しています。

## 複数人数モード

「人数モード」で1人／複数人（2〜6人）を切り替えます。人物タブで外見・衣装・表情・ポーズ・ロックを個別編集し、用途・画風・カメラ・背景・禁止事項は共通で設定します。識別名、画面基準の配置、関係性、全員の動作も指定できます。全員の動作を設定した場合は個別ポーズとその自由入力を保持したまま出力から除きます。

おまかせと4案生成は選択中の人物だけを変更し、共通設定と他の人物を維持します。1人モードでは選択中の人物を出力し、他の人物は保持します。背景用途では全員の人物設定を保持しつつ出力しません。保存・バックアップ・履歴・元に戻すは全員分に対応します。共有は識別名と人物別自由入力を初期状態で除外し、選択して含められます。長いリンクは共有用JSONで渡せます。画像生成サービス側で人数・配置が必ず再現されることを保証する機能ではありません。
